...
import {
    ...
    FaBrain, FaMicrophone, FaUserTie, FaCode, FaRocket, FaStar,
} from 'react-icons/fa'
                ...
                <div className="grid lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        ...
                        {project.tech?.length > 0 && (
                            <M.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.26 }}
                                className="glass-card rounded-2xl p-7">
                                <h2 className="text-lg font-bold text-white mb-5 flex items-center gap-2">
                                    <span className="w-1 h-5 rounded-full inline-block" style={{ background: project.accentColor }} />
                                    Tech Stack
                    ...
                    <div className="space-y-5">
                        ...
                        {project.url && (
                            <M.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }}
                                ...
                                <p className="text-white/50 text-xs mb-4 leading-relaxed">Production application serving real users.</p>
                                <a href={project.url} target="_blank" rel="noreferrer"
                                    ...
                                    style={{ background: project.accentColor }}>
                                    <FaExternalLinkAlt className="text-xs" /> Open {project.title}
                                </a>
                            </M.div>
                        )}

                        {others.length > 0 && (
                            <M.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.28 }}
                                className="glass-card rounded-2xl p-6">
                                <h3 className="text-sm font-bold text-white/60 uppercase tracking-widest mb-4">Other Projects</h3>
                                <div className="space-y-3">
                                    {others.map((op) => {
                                        const OtherIcon = iconComponents[op.iconName] ?? FaCode
                                        return (
                                            <Link key={op.slug} to={`/projects/${op.slug}`}
